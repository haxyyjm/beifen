<template>
  <div class="commercial-bill">
    <!--支付方式 -->
    <div class="shopping">
      <div  style="position: relative">
        <span style="vertical-align: top;display: inline-block;line-height: 35px;padding:10px;font-weight: bold">票据种类</span>
      ...
      <el-dialog
        ...
        <ul class="dialog_style">
          ...
          <!--<li v-show="Edite_false">-->
            <!--<span>连接名称 ：</span>-->
            <!--<el-input v-model="param.collection_name " placeholder="请输入内容" style="width: 200px;" ></el-input>-->
          <!--</li>-->
          <!--<li v-show="Edite">-->
            <!--<span>连接名称 ：</span>-->
            <!--<el-input v-model="param.collection_name " placeholder="请输入内容" style="width: 200px;" :disabled="true"></el-input>-->
          <!--</li>-->
          <li>
      ...
      <el-dialog
        ...
        <ul class="dialog_style">
          <li>
            <i style="color: red;font-size: 18px ">*</i>
            <span>字段名称 ：</span>
            <el-input v-model="param.field_name " placeholder="请输入内容" style="width: 217px;"></el-input>

          </li>
          <li>
            <span style="margin-left: 10px">字段中文名称 ：</span>
            <el-input v-model="param.field_name_cn " placeholder="请输入内容" style="width: 200px;"></el-input>
    ...
    export default {
      ...
      data() {
        return {
          ...
          Edite_false:false,//新增的时候显示的链接名称
          param :{},//新增入账代码里的内容

          //  入账代码的内容
          model_name :'',//单据种类
          code:'',//code
          // collection_name:'',//链接名称
          desc:'',//中文描述
          desc_en:'',//英文描述
          is_sys:'',//是否系统码
          is_group:'',//是否集团码
      ...
      methods: {
        ...
        screen(){
          let that=this
          that.$axios({
            url: that.api+"/v1/finance/bill_type/info_list",
            method: "post",
            data:{
        ...
        cancel(id,index,list){
          ...
          that.$confirm('此操作将永久删除该文件, 是否继续?', '提示', {
            ...
            list.splice(index, 1);
            that.$axios({
              url:""+that.api+"/v1/finance/bill_type/remove/"+id,
              method: "get",
              headers: {
                authorization: sessionStorage.getItem("authorization"),
              }
        ...
        amend(srow,id,index,list){
          ...
          console.log(srow);
          //  入账代码的内容
            that.param.model_name=srow.model_name, //
            that.param.code=srow.code,//code
              // that.param.collection_name=srow.collection_name,//链接名称
        ...
        //点击票据种类新增和修改后的保存
          ...
          if(!that.param.model_name ||!that.param.code ||!that.is_sys ||!that.is_group ||!that.is_halt){
            ...
            let urldata = that.Edite ? (that.api + "/v1/finance/bill_type/update/" + that.id) : (that.api + "/v1/finance/bill_type/add");
            ...
            that.$axios({
              ...
              data: {
                //  入账代码的内容
                model_name: that.param.model_name,//票据种类
                code: that.param.code,//代码
                // collection_name:that.param.collection_name,//链接名称
        ...
        * */
        ...
        fields_list(id,index,list){
          ...
          that.model_id=id;
          that.$axios({
            url: that.api+"/v1/finance/bill_type/get_fields",
            method: "post",
            data:{
        ...
        recompose(srow,id,index,list){
          ...
          that.param. field_name=srow.field_name, //字段名称
            that.param.field_name_cn=srow.field_name_cn,//字段中文名称
        ...
        augment(){
          ...
          if(!that.param.field_name || !that.field_type ||!that.param.field_desc){
            ...
            let urldata = that.Edite ? (that.api + "/v1/finance/bill_type/update_field/" + that.id) : (that.api + "/v1/finance/bill_type/add_field");
            console.info(urldata);
            that.$axios({
              url: urldata,
              method: "post",
              data: {
                ...
                field_name_cn: that.param.field_name_cn,//字段中文名称
