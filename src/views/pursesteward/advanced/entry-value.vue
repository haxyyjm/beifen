  ...
  <div class="entry-value">
    ...
    <div class="shopping">
      <div  style="position: relative">
        <span style="vertical-align: top;display: inline-block;line-height: 35px;padding:10px;font-weight: bold">入账代码</span>
        ...
        //  点击删除
          cancel(id,index,list){
            ...
            that.$confirm('此操作将永久删除该文件, 是否继续?', '提示', {
              ...
              list.splice(index, 1);
              that.$axios({
                url:""+that.api+"/v1/finance/incoming_account_code/remove/"+id,
                method: "get",
                headers: {
                  authorization: sessionStorage.getItem("authorization"),
                }
          ...
          amend(srow,id,index,list){
            ...
            //  入账代码的内容
              ...
              that.param.desc_en=srow.desc_en//英文描述
                if(srow.app_id===null){
                  that.app_id=srow.app_id
                }else {
                  that.app_id=srow.app_id.id//所属模块
                };
          ...
          //点击入账代码新增和修改后的保存
            ...
            let that=this;
            if(!that.in_or_out ||!that.app_id ||!that.param.code ||!that.param.name ||!that.is_sys ||!that.is_group ||!that.belong_to_balance ||!that.is_halt){
              that.hintInfo("warning", "*为必填项")
            }else {
              let urldata = that.Edite ? ("" + that.api + "/v1/finance/incoming_account_code/update/" + that.id) : ("" + that.api + "/v1/finance/incoming_account_code/add");
