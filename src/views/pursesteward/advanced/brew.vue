<template>
  ...
  <!--支付方式 -->
  <div class="shopping">
    <span style="vertical-align: top;display: inline-block;line-height: 35px;padding:10px;font-weight: bold">冲调帐原因</span>
    ...
    export default {
      ...
      methods: {
        ...
        enter_code(){
          let that=this
          that.$axios({
            url: ""+that.api+"/v1/finance/arrange_account_reason/info_list",
            method: "get",
            headers: {
        ...
        cancel(id,index,list){
          ...
          that.$confirm('此操作将永久删除该文件, 是否继续?', '提示', {
            ...
            list.splice(index, 1);
            that.$axios({
              url:""+that.api+"/v1/finance/arrange_account_reason/remove/"+id,
              method: "post",
              headers: {
                authorization: sessionStorage.getItem("authorization"),
              }
        ...
        //查账原因的新增和修改后的保存
          ...
          let that=this;
          if(!that.param.code || !that.is_sys || !that.is_group  ||!that.name){
            that.hintInfo("warning", "*为必填项")
          }else {
            let urldata = that.Edite ? ("" + that.api + "/v1/finance/arrange_account_reason/update/" + that.id) : ("" + that.api + "/v1/finance/arrange_account_reason/add");
