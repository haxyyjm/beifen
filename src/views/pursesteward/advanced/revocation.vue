<template>
  <div class="revocation">
    <!--支付方式 -->
    <div class="shopping">
      <span style="vertical-align: top;display: inline-block;line-height: 35px;padding:10px;font-weight: bold">撤销原因</span>
        ...
        <el-table
          ...
          <el-table-column
            ...
            <template slot-scope="scope">
              <span v-if="scope.row.the_type === 'transfer_account'">转账</span>
              <span v-else-if="scope.row.the_type === 'close_account'">平帐</span>
              <span v-else>冲调帐</span>
            </template>
          </el-table-column>
          <el-table-column
            prop="desc"
      ...
      <el-dialog
        ...
        <ul class="dialog_style">
          <li>
            ...
            <el-select v-model="the_type" placeholder="请选择" style="width: 200px;">
              <el-option
          ...
          <li>
            ...
            <el-input v-model="param.desc_cn" placeholder="请输入内容" style="width: 200px;"></el-input>
          </li>
          ...
          <span style="vertical-align: top">撤销原因 ：</span>
    ...
    export default {
      ...
      methods: {
        ...
        enter_code(){
          let that=this
          that.$axios({
            url: ""+that.api+"/v1/finance/accounts_cancel_reason/info_list",
            method: "get",
            headers: {
        ...
        cancel(id,index,list){
          ...
          that.$confirm('此操作将永久删除该文件, 是否继续?', '提示', {
            ...
            list.splice(index, 1);
            that.$axios({
              url:""+that.api+"/v1/finance/accounts_cancel_reason/remove/"+id,
              method: "post",
              headers: {
                authorization: sessionStorage.getItem("authorization"),
              }
        ...
        amend(srow,id,index,list){
          ...
          //  入账代码的内容
            ...
            that.param.desc_cn=srow.desc_cn,//英文描述
            that.is_sys=srow.is_sys+"",//是否系统码
            that.is_group=srow.is_group+""//是否集团码
             that.code=srow.code,//code
            that.name = srow.name//查账原因
               if(srow.the_type==='transfer_account'){
                 that.the_type='转账'
               }else if(srow.the_type==='close_account'){
                 that.the_type='平帐'
               }else {
                 that.the_type='冲调账'
               }
        ...
        //查账原因的新增和修改后的保存
          ...
          let that=this;
          if(! that.the_type || !that.is_sys || !that.is_group  ||!that.name){
            that.hintInfo("warning", "*为必填项")
          }else {
            let urldata = that.Edite ? ("" + that.api + "/v1/finance/accounts_cancel_reason/update/" + that.id) : ("" + that.api + "/v1/finance/accounts_cancel_reason/add");
            console.info(urldata);
            if (that.the_type === '转账') {
              that.the_type = ' transfer_account'
            } else if (that.the_type === '平帐') {
              that.the_type = ' close_account'
            } else if (that.the_type === '冲调帐') {
              that.the_type = 'arrange_account'
            }
            that.$axios({
              url: urldata,
              method: "post",
              data: {
                ...
                desc_cn: that.param.desc_cn,//英文描述
