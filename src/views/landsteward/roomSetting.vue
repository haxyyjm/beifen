...
<div class="site-all">
        ...
        <el-table
          ...
          <el-table-column
            ...
            <template slot-scope="scope">
              <el-button
                size="mini"
                @click="dialogVisibleFt =true; addAndUpdate = false; handleEdit(scope.row)">编辑</el-button>
              <el-button
                size="mini"
                type="danger"
                @click="handleDeleteFt(scope.row)">删除</el-button>
            </template>
          </el-table-column>
        </el-table>
        <el-pagination
  ...
  <el-dialog
    ...
    <ul>
      <li><span>*代码:</span><input v-model="param.code" maxlength="9"/><span>*代码名称:</span><input v-model="param.code_name"/></li>
      <li><span>创建用户:</span><input v-model="param.create_user"/><span>*描述:</span><input v-model="param.descript"/></li>
      <li><span>房间号:</span><input v-model="param.room_no" type="number"/><span>*楼层:</span><!--<input v-model="param.floor_number"/>-->
        <el-select v-model="param.floor_number" @focus="select_floor()" placeholder="请选择" size="mini" style="margin-left: 6px">
          <el-option
            v-for="item in options"
            :key="item.value"
            :label="item.label"
  ...
  <el-dialog
        ...
        class="dialog-style">
        <ul>
          <li><span>*房间状态:</span><input v-model="ft_param.code" maxlength="3"/><span>*说明:</span><input v-model="ft_param.desc"/></li>
          <li><span>*英文说明:</span><input v-model="ft_param.desc_en"/><span>*显示颜色:</span><input v-model="ft_param.color"/></li>
          <li><span>*是否停用:</span>
            <el-radio v-model="ft_param.is_halt" label='1'>是</el-radio>
            <el-radio v-model="ft_param.is_halt" label='0'>否</el-radio>
          </li>
          <li>
            <span>备注:</span>
...
<script>
    ...
    export default {
        ...
        methods:{
          ...
          select_floor(){
            let that = this;
            let url = 'http://'+that.url+'/room/get_roomfloor_list_tree/';
            that.$axios({
              method : 'get',
              url : url
            }).then(function (res) {
         ...
         */
          ...
          handleClick(tab, event) {
            ...
            //房态设置
              ...
              that.flagopenwin = tab.name;
              // let url = 'http://'+that.url+'/room_status/get_code_room_state_list/';
              let url = `http://192.168.4.99:8002/v1/room_status/get_code_room_state_list/`
              that.$axios({
                method : 'get',
                url : url,
                params:{
          ...
          handleEdit(row){
            ...
            //   authorization: that.authorization
            // }
            // let url = 'http://'+that.url+'/room_status/get_code_room_state_info/' + id + '/';
            let url = `http://192.168.4.99:8002/v1/room_status/get_code_room_state_info/${id}/`
            that.$axios.get(url).then(res=>{
                if(res.data.message == 'success'){
                    that.ft_param = res.data.data
                    that.ft_param.is_halt === 0 ? that.ft_param.is_halt = '0' : that.ft_param.is_halt = '1'
                }else{
          ...
          validateData(){
            return (
                ...
                util.validateBlank(this.ft_param.is_halt,'是否停用是必填项',this)
            )
        },
          handleDeleteFt(row){
            let that = this
            ...
            //   authorization: that.authorization
            // }
            //  let url = 'http://'+that.url+'/room_status/remove_code_room_state/' + id + '/';
            let url = `http://192.168.4.99:8002/v1/room_status/remove_code_room_state/${id}/`
          ...
          saveInfoStatus(){
            ...
            let that = this
            let id = that.rowId
            // let url = 'http://'+that.url+'/room_status/add_code_room_state/';
            // let url_2 = 'http://'+that.url+'/room_status/update_code_room_state/' + id + '/';
            let url = `http://192.168.4.99:8002/v1/room_status/add_code_room_state/`
            let url_2 = `http://192.168.4.99:8002/v1/room_status/update_code_room_state/${id}/`
            // let headers = {
            //   authorization: that.authorization
            // }
