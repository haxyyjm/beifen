<template>
    <div>
        <el-dialog width="50%" class="preview-enter-dialog" title="排房"  :visible.sync="chooseRoomVisible"
          ...
          <div style="height: 300px;overflow: auto">
            ...
            <span slot="footer" class="dialog-footer">
              <el-button style="height: 50px; width: 100px;" type="primary" @click="confirmChooseRoom()">确认排房</el-button>
          </span>
        </el-dialog>
        <!-- 选房号 -->
...
export default {
    ...
    watch: {
      show () {
        this.chooseRoomVisible = this.show;
        this.getRoomType() //准备:循环得到匹配的房型中文名
      },
      parentInfoParam(){
        ...
        console.log('this.preBillParam 父组件传来的',this.preBillParam)
       ...
       // this.arr_time = moment(this.preBillParam.reserve_base.leave_time[0]).format('YYYY-MM-DD')
        ...
        // this.previewEnterBill.room_no_value = this.preBillParam.reserve_rate[0].room_number//申明默认房间号
...
//  //封装支付宝二维码后的定时器 
      ...
      confirmChooseRoom(){
        // for(var item of this.preBillParam.reserve_guest){
        //   item.reserve_base_id = this.preBillParam.reserve_base.order_no
        // }
        // for(var item of this.preBillParam.reserve_rate){
        //   item.reserve_base_id = this.preBillParam.order_no
        // }
        ...
        console.log('scopeParam111111111',scopeParam)
        scopeParam.reserve_guest = []
        scopeParam.reserve_base = []
        scopeParam.room_list = []
        for(var item of scopeParam.reserve_rate){
          delete item.can_live_num
          delete item.room_type_value
          delete item.room_type
          delete item.room_class
          delete item.dynamic_roomNumber
          delete item.ratecode_id
          delete item.fix_rate
          delete item.room_count
          delete item.roomPrice
          delete item.list_order
          delete item.reserve_base_id
          delete item.checkin_date
          delete item.checkin_time
          delete item.code_room_special_ids
          delete item.code_room_facilities_ids
          delete item.is_arranged
          delete item.rmocc_ids
          delete item.add_person_num
          delete item.add_person_price
          delete item.add_bed_num
          delete item.add_bed_price
          delete item.code_pkg_id
          delete item.add_pkg_num
          delete item.add_pkg_price
          delete item.rate_service_fee
          delete item.rate_tax
          delete item.lower_price_id
          delete item.lower_price_reason
          delete item.lower_price_amount
          delete item.real_rate
          delete item.remark
          delete item.create_datetime
          delete item.create_user_id
          delete item.hotel_group_id
          delete item.hotel_id
          delete item.is_halt
          delete item.modify_date
          delete item.modify_user_id
          delete item.room_count
          delete item.room_price
          delete item. room_type_code
          delete item. room_type_descript
          delete item.room_type_descript_en
          delete item.roomCount
        }
        console.log('scopeParam2222222',scopeParam)
        that.$axios.post(url,scopeParam).then(res=>{
          if(res.data.message == 'success'){
            ...
            this.chooseRoomVisible = false
            this.$message.warning('排房失败!')
          }
          }).catch(error=>{
            this.$message.warning('接口有问题，待调整!')
            this.chooseRoomVisible = false
        })  
      },
      //排房后刷新orderlist
