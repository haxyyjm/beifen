<template>
  ...
  <div class="hp-all">
      ...
      <el-tab-pane  v-for="(item, index) in editableTabs2" :key="item.key" :label="item.title" :name="item.name"  :labelContent="item.labelContent" :closable="item.name+''!=='0' ? true : false">
        ...
        <div v-if="item.name+''!=='0'">
          <el-tabs v-model="editableTabsValueIn" type="card">
                ...
                <div class="early-style">
                  ...
                  <div class="early-style-right">
                    <ul>
                      ...
                      <li><span>引用自:</span><input v-model="param.parent_id" @click="dialogVisibleparent_id=true; select_info()"/>
            ...
            <div class="common-search">
              <span>检索条件:</span><input/><span>房型:</span><input/><span>显示内容:</span>
              <el-checkbox v-model="houseprice">房价</el-checkbox>
              <el-checkbox v-model="hpearly">包早</el-checkbox>
              <el-checkbox v-model="addbedprice">加床价</el-checkbox>
              <button>查询</button>
            </div>
            <div class="common-div">
              <el-table :data="commontableinfo" style="width: 100%" :header-cell-style="{background:'#303A41',color:'#FFFFFF'}" size="mini" height="200px">
                <!--表头-->
        ...
        <div v-else style="padding: 1%;padding-top: 0">
          ...
          <div style="margin-top: 1rem">
            <template>
              <div>
                <el-table @row-click="handleClick" v-loading="loading" @tab-click="handleChangeClick" :header-cell-style="{background:'rgb(48, 58, 65)',color:'#FFFFFF'}" :data="tableDatahpcode" size="mini" height="300px" style="width: 100%">
                  ...
                  <el-table-column prop="parent_id" label="引用自">
                  </el-table-column>
    ...
    <el-dialog :title="title" fullscreen :visible.sync="dialogVisibleHpCode" width="60%" class="dialog-style">
      <el-card class="card">
        <ul>
          ...
          <li><span>*开始日期:</span>
            <el-date-picker format="yyyy-MM-dd HH:mm:ss" value-format="yyyy-MM-dd HH:mm:ss" v-model="start_date" size="mini" style="width: 200px;margin-left: 10px" type="datetime" placeholder="选择日期时间"></el-date-picker>
            <span>*结束日期:</span>
            <el-date-picker format="yyyy-MM-dd HH:mm:ss" value-format="yyyy-MM-dd HH:mm:ss" v-model="end_date" size="mini" style="width: 200px;margin-left: 10px" type="datetime" placeholder="选择日期时间"></el-date-picker></li>
        </ul>
        <ul>
          <li>
            <span>*类别:</span>
            <el-select v-model="param.category" @focus="select_info('category')" placeholder="请选择" size="mini" style="margin-left: 6px">
              <el-option v-for="item in options_category" :key="item.value" :label="item.descript" :value="item.code">
              </el-option>
            </el-select>
            <span>包价:</span>
            <el-select v-model="param.packages" @focus="select_info('packages')" placeholder="请选择" size="mini" style="margin-left: 6px">
          ...
          <li>
            <span>佣金码:</span>
            <el-select v-model="param.cms_code" @focus="select_info('cms_code')" placeholder="请选择" size="mini" style="margin-left: 6px">
              <el-option v-for="item in options_cms_code" :key="item.value" :label="item.descript" :value="item.code">
              </el-option>
            </el-select>
            <span>客房布置:</span>
            <el-select multiple collapse-tags v-model="param.amenities" @focus="select_info('amenities')" placeholder="请选择" size="mini" style="margin-left: 6px">
          ...
          <li>
            <span>市场码:</span>
            <el-select v-model="param.market" @focus="select_info('market')" placeholder="请选择" size="mini" style="margin-left: 6px">
              <el-option v-for="item in options_market" :key="item.value" :label="item.descript" :value="item.code">
              </el-option>
            </el-select>
            <span>来源码:</span>
            <el-select v-model="param.src" @focus="select_info('src')" placeholder="请选择" size="mini" style="margin-left: 6px">
          ...
          <li>
            ...
            <span>基础房价码设置:</span>
            <el-select v-model="param.parent_code" @focus="select_info('parent_code')" placeholder="请选择" size="mini" style="margin-left: 6px">
              <el-option v-for="item in options_parent_code" :key="item.value" :label="item.label" :value="item.parent_code">
              </el-option>
            </el-select>
            <span v-show="maskparent">引用自:</span>
            <el-select v-show="maskparent" v-model="param.parent_id" @focus="select_info('parent_id')" placeholder="请选择" size="mini" style="margin-left: 6px">
              <el-option v-for="item in options_parent_id" :key="item.value" :label="item.descript" :value="item.code">
              </el-option>
            </el-select>
          </li>
          ...
          <li v-show="maskextendli">
            <!--这里是新建房价码内容的设置-->
            <span>四舍五入位数:</span>
            <el-select v-model="SizeChoice" placeholder="请选择" size="mini" style="margin-left: 6px">
              <el-option v-for="item in RoomSizeChoice" :key="item.value" :label="item.label" :value="item.key">
              </el-option>
            </el-select>
            <span>四舍五入末尾数:</span>
            <el-select v-model="TailChoice" @focus="select_info('round_tail')" placeholder="请选择" size="mini" style="margin-left: 6px">
          ...
          <li v-show="maskextendli">
            <!--这里是新建房价码内容的设置-->
            <span>加法系数:</span><input type="number" />
            <span>乘法系数:</span><input/>
          </li>
        </ul>
      </el-card>
    ...
    <el-dialog title="引用自" fullscreen :visible.sync="dialogVisibleparent_id" width="30vw" :style="{height:'50vw'}" top="20vh"  class="dialog-style">
      <el-table
        :data="tableDataparent_id"
        size="mini"
        @row-click="select_parent_id"
        :header-cell-style="{background:'#68819E',color:'#FFFFFF'}"
        style="width: 100%">
        <el-table-column
          prop="code"
    ...
    <el-dialog title="快速设置" fullscreen :visible.sync="dialogVisibleafastSet" width="30vw" :style="{height:'50vw'}" top="20vh"  class="dialog-style">
      <el-table
        ...
        <el-table-column
          ...
          label="房类">
          <template slot-scope="scope">
            <span style="cursor: pointer" @click="setRow(scope.row,scope.$index)" type="text" size="mini">{{scope.row.room_types+"→"}}</span>
          </template>
        </el-table-column>
        <el-table-column
          prop="week0"
        ...
        <el-table-column
          ...
          label="周三">
          <template slot-scope="scope">
            <el-input v-model="scope.row.week3" @input="SetValue($event,scope.$index)" size="mini" style="width: 6vw"></el-input>
          </template>
        </el-table-column>
        <el-table-column
          prop="week4"
      ...
      <span slot="footer" class="dialog-footer">
        ...
        <button type="primary" @click="dialogVisibleafastSet = false;fast_set()">确 定</button>
      </span>
    </el-dialog>
  </div>
</template>
    ...
    methods: {
      ...
      SetValue(val,index){
         ...
         that.curr_index = index;
          ...
          this.week6Val='';
          this.inputVal = val
         that.check_number(val);
         //console.info(val+"this is value");
      },
      /**
       * @fast_set 提交快速设置的信息
       */
      fast_set(){
        ...
        for(let i in that.tableDatafastSet){
          tableDatafastSet.push(that.tableDatafastSet[i]);
        }
        let final_data = [];
        ...
        for(let i in tableDatafastSet){
          ...
          dataobj = final_data.map((res,index)=>{
              return {
                roomtype : res.room_types,
              }
          });
          console.info(dataobj[0].roomtype);
          ...
          that.$axios({
            ...
            data :{
              ...
              begin_date : that.start_date,
              end_date : that.end_date,
              room_types : dataobj[0].roomtype,
      ...
      check_number(val){
        let that = this;
        /*val = parseInt((val=val.replace(/\D/g,''))==''?'0':val,10);*/
        that.reSetValue = parseInt(val.replace(/[^0-9-]+/,'')).toFixed(2);
        //hat.reSetValue = parseInt((val=val.replace(/\D/g,''))==''?'0':val,10);
       ...
       * @param no
       */
      /*setColumn(column,event){
/!*        console.log(this.inputVal)*!/
        console.info(column);
        if(column.property+'' === 'week0'){
          this.week0Val = parseInt(this.inputVal.replace(/[^0-9-]+/,'')).toFixed(2);
        }else if(column.property+'' === 'week1'){
          this.week1Val = parseInt(this.inputVal.replace(/[^0-9-]+/,'')).toFixed(2);
        }else if(column.property+'' === 'week2'){
          this.week2Val = parseInt(this.inputVal.replace(/[^0-9-]+/,'')).toFixed(2);
        }else if(column.property+'' === 'week3'){
          this.week3Val = parseInt(this.inputVal.replace(/[^0-9-]+/,'')).toFixed(2);
        }else if(column.property+'' === 'week4'){
          this.week4Val = parseInt(this.inputVal.replace(/[^0-9-]+/,'')).toFixed(2);
        }else if(column.property+'' === 'week5'){
          this.week5Val = parseInt(this.inputVal.replace(/[^0-9-]+/,'')).toFixed(2);
        }else if(column.property+'' === 'week6'){
          this.week6Val = parseInt(this.inputVal.replace(/[^0-9-]+/,'')).toFixed(2);
        }
      ...
      hint_sync(row) {
        let that = this;
        this.$confirm('指定的数据不在当前行，是否继续？', '同步警告！！！', {
      ...
      flushSet(){
       ...
       let url = 'http://'+that.url + '/rate_code/get_rate_code_log_list/';
        that.$axios({
            method : 'get',
            url : url,
            params:{
          ...
          console.info(res);
           that.tableDatafastSet = res.data.data.results;
        }).catch((err)=>{
          console.error(err);
        })
      },
      ...
      select_info: function(flag_char) {
        ...
        if(flag_char == 'category') {
          ...
          //入账代码
          urlData = 'http://47.98.113.173:8095/v1/system/settings/get_code_transaction_list/';
          ...
          that.fatherData({
            ...
            that.options_parent_id = res;
            that.tableDataparent_id = res;
          });
        }
      },
      /**
      ...
      handleClick(val) {
        ...
        if(this.editableTabs2 && this.editableTabs2.length > 0) {
          ...
          if(!isHave) {
            ...
            that.searchInfo_Details(val.id);
            that.change_days();
          } else {
       ...
       */
      select_parent_id(row){
        let that = this;
        that.param.parent_id = row.code;
      ...
      SelectionfastSet(selection){
        ...
        let that = this;
        for(let i in selection){
          weeks.push(selection[i].code);
        }
        that.param.weeks = weeks;
      },
      /**
       * @ setTitle 设置新增头部信息
       */
      ...
      handleEditClick: function(tab, event) {
        ...
        if(tab.active == true) {
          ...
          if(tab.label === '房价码列表'){
            ...
            that.searchInfo_Details(tab.labelContent);
          }
