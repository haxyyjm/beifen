        ...
        <div class="mask-body">
          <ul>
            ...
            <li><span>起始时间:</span>
              ...
              <span>结束时间:</span>
              <el-date-picker format="yyyy-MM-dd HH:mm:ss" value-format="yyyy-MM-dd HH:mm:ss" v-model="closing_time" size="mini" style="width: 200px;margin-left: 2px" type="datetime" placeholder="选择日期时间">
              </el-date-picker>
            </li>
            <li><span>描述:</span><input v-model="descript" /><span>英文描述:</span><input v-model="descript_en" /></li>
            <li><span>对应集团代码:</span><input v-model="group_code" /><span>销售价格:</span><input v-model="price" /></li>
            <li><span>成本:</span><input v-model="cost" /></li>
            <li style="margin-left: -30px"><span>是否集团代码:</span>
              ...
              <span>是否停用:</span>
              <el-radio v-model="is_half" label="1">是</el-radio>
              <el-radio v-model="is_half" label="2">否</el-radio>
             <!-- <span>是否系统代码:</span>
      ...
      <div class="mask-suspen" v-show="maskShowupdate">
        ...
        <div class="mask-body">
          <ul>
            <li><span>代码:</span><input v-model="code" /><span>早餐份数:</span><input v-model="quantity" type="number" /></li>
            <li><span>n天后生效起始日期:</span>
              <el-date-picker format="yyyy-MM-dd HH:mm:ss" value-format="yyyy-MM-dd HH:mm:ss" v-model="starting_days" size="mini" style="width: 200px;margin-left: 6px" type="datetime" placeholder="选择日期时间">
              </el-date-picker>
              <span>n天后生效结束日期:</span>
              <el-date-picker format="yyyy-MM-dd HH:mm:ss" value-format="yyyy-MM-dd HH:mm:ss" v-model="closing_days" size="mini" style="width: 200px;margin-left: 2px" type="datetime" placeholder="选择日期时间">
              </el-date-picker>
            </li>
            <li><span>起始时间:</span>
              <el-date-picker format="yyyy-MM-dd HH:mm:ss" value-format="yyyy-MM-dd HH:mm:ss" v-model="starting_time" size="mini" style="width: 200px;margin-left: 6px" type="datetime" placeholder="选择日期时间">
...
<script>
  export default {
    ...
    methods: {
      ...
      searchall: function() {
        let that = this;
        let url = 'http://'+that.url+'/rate_code/get_breakfast_list/';
        this.$axios({
          method: 'get',
          url: url,
        }).then(function(res) {
      ...
      inquire: function() {
        ...
        that.checkis_half == true ? is_half = 1 : is_half = "";
        if(count == "" && free == "" && expired == "" && is_half == "") {
          that.searchall()
        } else {
          this.$axios({
            method: 'get',
      ...
      addEarlyInfo: function() {
        let that = this;
        let url = 'http://'+that.url+'/rate_code/add_breakfast/';
        //let is_group = that.is_group == '1' ? true : false;
        //let is_half = that.is_half == '1' ? true : false;
      ...
      editEarlyInfo: function(model, index) {
        ...
        let is_half = model.is_half == true ? '1' : '2';
      ...
      updateEarlyInfo: function() {
        ...
        let is_half = that.is_half == '1' ? true : false;
        let is_sys = that.is_sys == '1' ? true : false;
        let url = 'http://' + that.url + '/breakfast/' + that.id + '/renew/';
        this.$axios({
          method: 'post',
          url: url,
          data: {
