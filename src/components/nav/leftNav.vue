<template>
  ...
  <!-- <el-col style="width: 240px;" class="main"> -->
    <!--左侧导航-->
        ...
        <template v-for="issue in $router.options.routes">
          <template v-if="issue.name === $store.state.leftNavState"><!-- 注意：这里就是leftNavState状态作用之处，当该值与router的根路由的name相等时加载相应菜单组 -->
            <template v-for="(item,index) in issue.children">
              <el-submenu v-if="!item.leaf" :key="item.path" :index="index+''" v-show="item.menuShow">
              ...
              <el-menu-item v-else-if="item.leaf&&item.children&&item.children.length" :key="item.path" :index="item.children[0].path"
