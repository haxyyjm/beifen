...
        return encodeURI(data).replace(/&/g,'%26').replace(/\+/g,'%2B').replace(/\=/g, '%3D').replace(/\s/g,'%20').replace(/#/g,'%23');  
       ...
       */
        ...
        postEnter(){
          ...
          this.getPriceFromType(scopeParam) //放在前面
        ...
        getRateCodeAll(item){
          ...
          let scopeParam ={
            ...
            begin_date:  moment(new Date(item.master_base[0].leave_time[0])).format('YYYY-MM-DD'),
            end_date: moment(new Date(item.master_base[0].leave_time[1])).format('YYYY-MM-DD'),
            room_type_list: []
          }
          that.$axios.post(url,scopeParam).then(res=>{
            if(res.data.message == 'success'){
